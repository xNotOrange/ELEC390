...
class OneWayLeft(TrafficObject):

    def set_car_state(self, car_state):
        logging.debug('left one way: turn left')
        # steer angle -30 degrees
        car_state['steering_angle'] = -30
        # reduce speed
        car_state['speed'] = car_state.get('speed', 0) * 0.8
        
        
class OneWayRight(TrafficObject):

    def set_car_state(self, car_state):
        logging.debug('right one way: turn right')
        # steer angle -30 degrees
        car_state['steering_angle'] = 30
        # reduce speed
        car_state['speed'] = car_state.get('speed', 0) * 0.8
...
class DoNotEnter(TrafficObject):

    def set_car_state(self, car_state):
        logging.debug('do not enter: stopping car')
